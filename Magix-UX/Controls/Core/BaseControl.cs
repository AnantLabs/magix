...
namespace Magix.UX.Widgets.Core
{
	...
	public abstract class BaseControl : Control
	{
         ...
         * need some modified logic. See how the Aspects work for a working example of
         * how this method can be overridden.
         */
         ...
         */
		public virtual IEnumerable<AspectBase> Aspects
		{
			get
			{
				foreach (Control idx in Controls)
				{
                    AspectBase asp = idx as AspectBase;
					if (asp != null)
						yield return asp;
				}
			}
        ...
        protected virtual string GetClientSideScript()
		{
            ...
            string behaviors = GetRegisterAspectsScript();
            if (!string.IsNullOrEmpty(behaviors))
                behaviors = "aspects:" + behaviors;
            ...
            optionsString = StringHelper.ConditionalAdd(behaviors, "", ",", optionsString);
        ...
        // This method will create the 'register aspects script' for your widget.
        // Basically just a helper method being called by the framework itself.
		private string GetRegisterAspectsScript()
		{
			string retVal = "";
			foreach (AspectBase idx in Aspects)
			{
                if (idx.Visible)
                {
                    retVal = StringHelper.ConditionalAdd(
                        retVal, 
                        "", 
                        ",", 
                        idx.GetAspectRegistrationScript());
                }
			}
