...
namespace Magic.Brix.Components
{
    public class Grid : Panel
    {
        ...
        {
            get { return ViewState["Selectable"] == null ? false : (bool)ViewState["Selectable"]; }
        ...
        public IEnumerable<int> SelectedRows
        {
            get
            {
                foreach (CheckBox idx in Selector.Select<CheckBox>(_lstWrappers,
                    delegate(Control idx2)
                    {
                        return idx2.ID.IndexOf("chkBtn") == 0;
                    }))
                {
                    if (idx.Checked)
                    {
                        yield return int.Parse(idx.Info);
                    }
                }
            }
        }
        ...
        private void CreateRows(Magic.UX.Widgets.Core.BaseWebControl table)
        {
            ...
            foreach (Node idxRow in node)
            {
                if (PageSize == -1 || idxStartBinding >= CurrentPage * PageSize)
                {
                            ...
                            case "LinkButton":
                                {
                                    LinkButton lb = new LinkButton
                                    {
                                        ...
                                        Info = idxRow["ID"].Value + "|" + idxCell.Name,
                                    ...
                                    if (Action != null)
                                    {
                                        lb.Click +=
                                            delegate(object sender, EventArgs e)
                                            {
                                                ...
                                                if (rl == null)
                                                    return;
                                                string id = rl.Info.Split('|')[0];
                                                string columnName = rl.Info.Split('|')[1];
                                    ...
                                    if (CellEdited != null)
                                    {
                                        edit.TextChanged +=
                                            delegate(object sender, EventArgs e)
                                            {
                                                ...
                                                if (ed != null)
                                                {
                                                    string id = ed.Info.Split('|')[0];
                                                    string cellName = ed.Info.Split('|')[1];
                            ...
                            default:
                                {
                                    if (fieldType.IndexOf("TextAreaEdit") == 0)
                                    {
                                        ...
                                        edit.CssClass = "mux-in-place-edit";
                                        edit.Info = idxRow["ID"].Value + "|" + idxCell.Name;
                    ...
                    if (Selectable)
                    {
                        HtmlTableCell sel = new HtmlTableCell { ID = "sel" + idxNo };
                        CheckBox lb = new CheckBox
                        {
                            ID = "chkBtn" + idxNo,
                            CssClass = "chkBtn",
                            Info = idxRow["ID"].Value.ToString()
                        };
                        sel.Controls.Add(lb);
                    ...
                    if (RowDeleted != null)
                    {
                        ...
                        lb.Click +=
                            delegate(object sender, EventArgs e)
                            {
                                ...
                                if (bt != null)
                                {
                                    RowDeleted(this, new GridActionEventArgs(bt.Info, null));
                                    if (AutoDeleteRowOnDeletion)
                                    {
                                        foreach (Node idx in DataSource["Rows"])
                                        {
                                            if (idx["ID"].Value.ToString() == bt.Info)
                                            {
        ...
        private void CreateHeaders(Control table)
        {
            ...
            foreach (Node idx in DataSource["Columns"])
            {
                ...
                if (SortColumn == btn.Info)
                {
                    btn.CssClass = "button headerButton clicked";
                }
                else if (SortColumn == "-" + btn.Info)
                {
                ...
                btn.Click +=
                    delegate(object sender, EventArgs e)
                    {
                        ...
                        if (bt != null)
                        {
                            bool backwards = SortColumn == bt.Info;
                            SortColumn = backwards ? "-" + bt.Info : bt.Info;
                        }
                        DataBindGrid();
                        _lstWrappers.ReRender();
                    };
